...
<img src="./sasie/D10_fetch_magic.png" width="300" />
...
### ✂️ 2回目の await： `response.json()`
  ...
  * **理由：** 届いたデータはまだ「ただの文字の羅列」なので、それをプログラムで使える形に直すのに少し時間がかかるのです。
...
「JavaScriptのオブジェクト」は、JavaScriptにしか通言葉です。
...
graph LR
    ...
    style JS fill:#f9f
    style JSON fill:#fff,stroke:#333,stroke-width:2px,stroke-dasharray: 5 5
    style Py fill:#ccf
    style Java fill:#ccf
    style PHP fill:#ccf
```
